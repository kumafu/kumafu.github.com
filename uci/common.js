...
function RGBtoWEB(string){
  var nums = string.match(/\(.*\)/);
  var arr = nums[0].replace("(","").replace(")","").split(",");
  var r = ("0" + Number(arr[0]).toString(16)).slice(-2);
  var g = ("0" + Number(arr[1]).toString(16)).slice(-2);
  var b = ("0" + Number(arr[2]).toString(16)).slice(-2);
  return "" + r + g + b;
}

function WEBtoRGBA(a,colorString){
